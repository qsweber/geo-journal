...
//
// This identity was originally created and managed by the geo-journal-web frontend's Pulumi
// stack, tied to the frontend's own domain - it's relocated here since Cognito (which sends
// verification email through it) now lives in this backend stack instead.
func createSESResources(scope constructs.Construct, cfg EnvConfig) *SESResources {
	...
	emailIdentity := awsses.NewCfnEmailIdentity(scope, jsii.String("SESEmailIdentity"), &awsses.CfnEmailIdentityProps{
		EmailIdentity: jsii.String(domain),
	})
	...
	verificationRecord := awsroute53.NewCfnRecordSet(scope, jsii.String("SESDomainVerification"), &awsroute53.CfnRecordSetProps{
		Name:            jsii.String(fmt.Sprintf("_amazonses.%s", domain)),
