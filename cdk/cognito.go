...
// createCognitoResources relocates the Cognito user pool + client that were originally created
// by the geo-journal-web frontend's Pulumi stack. They're named after the frontend's domain
// (not this backend's), since that's what they were created as and must match for `cdk import`.
func createCognitoResources(scope constructs.Construct, cfg EnvConfig, sesIdentity awsses.CfnEmailIdentity) *CognitoResources {
	domain := cfg.FrontendDomainName

	sesSourceArn := awscdk.Stack_Of(scope).FormatArn(&awscdk.ArnComponents{
		Service:      jsii.String("ses"),
		Resource:     jsii.String("identity"),
		ResourceName: jsii.String(domain),
	})

	userPool := awscognito.NewCfnUserPool(scope, jsii.String("UserPool"), &awscognito.CfnUserPoolProps{
		UserPoolName: jsii.String(domain + "-user-pool"),
		...
		EmailConfiguration: &awscognito.CfnUserPool_EmailConfigurationProperty{
			...
			From:                jsii.String("\"" + domain + "\" <no-reply@" + domain + ">"),
		...
		VerificationMessageTemplate: &awscognito.CfnUserPool_VerificationMessageTemplateProperty{
			...
			EmailSubject:       jsii.String("Verify your email with " + domain),
		},
	})
	...
	userPoolClient := awscognito.NewCfnUserPoolClient(scope, jsii.String("UserPoolClient"), &awscognito.CfnUserPoolClientProps{
		...
		ClientName:                 jsii.String(domain + "-user-pool-client"),
