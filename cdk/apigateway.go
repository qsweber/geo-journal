...
func createAPIGatewayResources(
	...
	envVars := map[string]*string{
		"GEO_JOURNAL_UPLOAD_BUCKET": jsii.String(cfg.UploadBucketName),
		"COGNITO_REGION":            jsii.String(cfg.Region),
		"COGNITO_USER_POOL_ID":      cognitoResources.UserPool.AttrUserPoolId(),
		"COGNITO_CLIENT_ID":         cognitoResources.UserPoolClient.AttrClientId(),
	}

	asset := awss3assets.NewAsset(scope, jsii.String("FunctionAsset"), &awss3assets.AssetProps{
		Path: jsii.String("../handler.zip"),
	})

	function := awslambda.NewCfnFunction(scope, jsii.String("Function"), &awslambda.CfnFunctionProps{
		FunctionName: jsii.String(cfg.LambdaName),
